//
//  CZSendRequestViewController.m
//  FilmSquad
//
//  Created by 陈卓 on 16/12/22.
...
@interface CZSendRequestViewController ()<UITableViewDelegate,UITableViewDataSource>

@property (nonatomic,strong) UITableView      *tableView;
@property (nonatomic,strong) YYTextView       *requesInfoTextView;      //输入请求信息的textView
...
@implementation CZSendRequestViewController
#pragma mark - 懒加载
-(UITableView *)tableView{
    if(!_tableView){
        ...
        _tableView.tableHeaderView = self.requesInfoTextView;
...
-(YYTextView *)requesInfoTextView{
    if(!_requesInfoTextView){
        _requesInfoTextView                      = [[YYTextView alloc]initWithFrame:CGRectMake(0, 0, kWIDTH, kWIDTH * 0.618)];
        _requesInfoTextView.placeholderText      = @"请输入请求信息";
        _requesInfoTextView.placeholderFont      = YuanFont(16);
        _requesInfoTextView.font                 = YuanFont(16);
        _requesInfoTextView.alwaysBounceVertical = YES;
    }
    return _requesInfoTextView;
}
...
-(void)viewDidLoad {
    [super viewDidLoad];
    self.view.tag = 1;
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"发送" style:(UIBarButtonItemStylePlain) target:self action:@selector(sendRequest:)];
    [self masonry];
}
#pragma mark - masonry
-(void)masonry{
...
//发送好友请求
-(void)sendRequest:(UIBarButtonItem*)sender {
    [SVProgressHUD showWithStatus:@"正在发送"];
    BmobUser *user = [BmobUser currentUser];
    //获取我的好友请求列表
    NSMutableArray *allMyRequest = [[user objectForKey:@"MyRequest"] mutableCopy];
    if(allMyRequest == nil){
        allMyRequest = [NSMutableArray array];
    }
    NSArray *friendInfo = @[self.user.objectId, self.selectedSquad,self.remarkName, @"申请中"];
    [allMyRequest addObject:friendInfo];
    [user setObject:allMyRequest forKey:@"MyRequest"];
    [user updateInBackgroundWithResultBlock:^(BOOL isSuccessful, NSError *error) {
        if(isSuccessful){
            //环信管理，发送加好友申请
            BOOL successful = [[EaseMob sharedInstance].chatManager addBuddy:self.user.objectId message:self.requesInfoTextView.text error:nil];
            if(successful){
                [SVProgressHUD showSuccessWithStatus:@"发送请求成功"];
                [self.navigationController popToRootViewControllerAnimated:YES];
            }else{
                [SVProgressHUD showErrorWithStatus:@"发送请求失败，请检查网络"];
            }
